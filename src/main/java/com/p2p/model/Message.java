...
public class Message {
    ...
    // ================= FACTORY: CALL =================
    public static Message callRequest(Peer from, Peer target, boolean isVideo) {
        Message m = new Message(Type.CALL_REQUEST, from.getId(), from.getName());
        m.target = target.getId();
        m.content = isVideo ? "VIDEO_CALL" : "VOICE_CALL";
        return m;
    }

    public static Message callAccept(Peer from, Peer target) {
        Message m = new Message(Type.CALL_ACCEPT, from.getId(), from.getName());
        m.target = target.getId();
        return m;
    }

    public static Message callReject(Peer from, Peer target) {
        Message m = new Message(Type.CALL_REJECT, from.getId(), from.getName());
        m.target = target.getId();
        return m;
    }

    public static Message callEnd(Peer from, Peer target) {
        Message m = new Message(Type.CALL_END, from.getId(), from.getName());
        m.target = target.getId();
        return m;
    }
    ...
    public boolean isVideoCall() {
        return "VIDEO_CALL".equals(content);
    }

    public boolean isVoiceCall() {
        return "VOICE_CALL".equals(content);
    }
