...
import com.p2p.controller.CallController;
...
public class MainController {
    ...
    private void initiateCall(boolean isVideo) {
        ...
        if (voiceCallService.isInCall()) {
            showAlert("Call in progress", "Please end the current call first");
            return;
        }

        Peer target = findPeerById(selectedConversation.getId());
        if (target == null || target.getAddress() == null) {
            showAlert("Cannot make call", "User is offline");
            return;
        }

        voiceCallService.initiateCall(target, isVideo);
        showCallDialog(target, false, isVideo);
    }
    ...
    private void setupCallbacks() {
        ...
        voiceCallService.setOnIncomingCall(msg -> Platform.runLater(() -> {
            Peer caller = findPeerById(msg.getFrom());
            if (caller != null) {
                boolean isVideo = msg.isVideoCall();
                showCallDialog(caller, true, isVideo);
            }
        }));

        voiceCallService.setOnCallAccepted(msg -> Platform.runLater(() -> {
            showNotification("Call Accepted", "Call connected");
        }));

        voiceCallService.setOnCallRejected(msg -> Platform.runLater(() -> {
            showNotification("Call Rejected", "The call was rejected");
            closeCallDialog();
        }));

        voiceCallService.setOnCallEnded(msg -> Platform.runLater(() -> {
...
// ================= RECALL =================
    ...
    private void handleSignal(Message msg) {
        ...
        // Forward call signals to VoiceCallService
        if (msg.getType() == Message.Type.CALL_REQUEST ||
            ...
            if (sender != null && sender.getAddress() != null) {
                voiceCallService.handleCallSignal(msg, sender.getAddress());
            }
            return;
        }
    ...
    // ================= CALL DIALOG =================
    private void showCallDialog(Peer peer, boolean isIncoming, boolean isVideo) {
        try {
            FXMLLoader loader = new FXMLLoader(
                    getClass().getResource("/com/p2p/view/call_dialog.fxml")
            );

            Scene scene = new Scene(loader.load());
            callController = loader.getController();

            callDialogStage = new Stage();
            callDialogStage.setScene(scene);
            callDialogStage.setTitle(isIncoming ? "Incoming Call" : "Calling...");
            callDialogStage.initModality(Modality.APPLICATION_MODAL);
            callDialogStage.initOwner(conversationListView.getScene().getWindow());
            callDialogStage.setResizable(false);
            callDialogStage.setOnCloseRequest(e -> {
                if (voiceCallService.isInCall()) {
                    voiceCallService.endCall();
                }
            });

            callController.setContext(
                    callDialogStage,
                    peer,
                    localPeer,
                    voiceCallService,
    ...
    private void closeCallDialog() {
        if (callDialogStage != null) {
            Platform.runLater(() -> callDialogStage.close());
            callDialogStage = null;
            callController = null;
        }
    }
